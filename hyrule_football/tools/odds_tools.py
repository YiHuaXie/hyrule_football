...
from hyrule_football.schema import BasedMatchOddsInfo, MatchInfo, Company
from hyrule_football.service.match_service import get_match_for_name
...
def plan_match_odds_query(match_input: MatchOddsInput) -> Optional[MatchOddsPlan]:
    ...
    company_list = [
        company
        for company in (Company.from_name(name) for name in match_input.company_list)
